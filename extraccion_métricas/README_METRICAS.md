...
| Columna                          | Tipo           | Descripción                                                                                                                      | Interpretación del valor                                                                                                                          |
| -------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| **amf**                          | Métrica        | AMF – *Attribute Match Frequency*. Proporción de hoteles consumidos que tienen este atributo.                                    | **Mayor es mejor.** 1 = todos los consumidos lo tienen → indica preferencia fuerte.                                                               |
| **attribute_frequency**          | Métrica        | TF – Frecuencia absoluta del atributo en los consumidos.                                                                         | **Mayor es mejor.** Representa la familiaridad del usuario con el atributo.                                                                       |
| **attribute_tfidf**              | Métrica        | TF-IDF del atributo para el usuario. Destaca atributos frecuentes para el usuario pero raros en el catálogo global.              | **Mayor es mejor.** Valores altos indican preferencias distintivas.                                                                               |
| **attribute_contribution_score** | Métrica        | Qué tanto contribuye el atributo a justificar la recomendación. Calculado como: `frecuencia_usuario × presencia_en_recomendado`. | **Mayor es mejor.** Indica cuánto apoya el atributo la explicación.                                                                               |
| **attribute_presence_ratio**     | Métrica global | APR: proporción de atributos del recomendado que el usuario ya ha encontrado antes.                                              | **Mayor es mejor.** Mide familiaridad general con el recomendado.                                                                                 |
| **attribute_overlap_count**      | Métrica global | Número absoluto de atributos compartidos entre el usuario y el recomendado.                                                      | **Mayor = más similitud** entre gustos y recomendado.                                                                                             |
| **attribute_overlap_ratio**      | Métrica global | Jaccard: intersección / unión de atributos entre usuario y recomendado.                                                          | **Mayor es mejor.** Captura similitud relativa.                                                                                                   |
| **attribute_novelty**            | Métrica        | Indica si el atributo es nuevo para el usuario. 1 = nuevo, 0 = ya visto.                                                         | **No es “mejor/peor”.** 1 = novedad → sorpresa; 0 = familiaridad.                                                                                 |
| **attribute_specificity**        | Métrica        | Especificidad = 1 / popularidad del atributo en el catálogo.                                                                     | **Mayor es mejor.** Valores altos = atributo raro, más explicativo.                                                                               |
| **attribute_stability**          | Métrica        | TF del atributo normalizado sobre el atributo más frecuente del usuario.                                                         | **Mayor es mejor.** Indica gustos estables/consistentes.                                                                                          |
| **attribute_variability**        | Métrica        | Variabilidad dentro del *property_type* en los consumidos. Varía entre 0 y 1.                                                    | **Baja = el usuario suele consumir el mismo valor**, **Alta = consume valores diferentes**. No es estrictamente bueno o malo; describe el perfil. |
